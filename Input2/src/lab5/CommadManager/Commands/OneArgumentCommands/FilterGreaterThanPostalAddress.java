...
import lab5.Organization.OrganizationType;

import java.util.List;
import java.util.Locale;
import java.util.stream.Collectors;

public class FilterGreaterThanPostalAddress extends CommandOne<String>{
    @Override
    public String execute(String arg) {
        try {
            OrganizationType.valueOf(arg.toUpperCase(Locale.ROOT));
        } catch (IllegalArgumentException e){ return "Wrong Type";}
        List<Organization> list = CollectionManager.getCollection().stream().filter(p -> p.getPostalAddress().getStreet().compareTo(arg) > 0).collect(Collectors.toList());
        String response = "";
